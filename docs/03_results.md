...
HOMER produces a probabilistic mouse↔human parcel coupling π (1,864 × 2,094) — sharp/peaked at the production regularisation, see `01_overview.md` — via Fused Gromov-Wasserstein optimal transport, supervised on 21 Garin 2021 point anchors + 26 region-anchor entries from 15 cross-species packs. On Beauchamp 2022's external 22-pair gene-expression benchmark, the recommended π hits 45.7 % parcel-level top-1 (50.6× over null) and 100 % qualified top-3 at region level, with 98.2 % bootstrap stability and +17.8 σ above the permuted-anchor null. It was tested against twelve independent cross-species datasets spanning FC, gene expression, cell types, cortical layers, connectivity gradients, intracortical myelin, and psychiatric disorders, plus two negative-control tests (frontal-cortex homology and the tethering hypothesis) and a head-to-head comparison with the TransBrain method. Translation is robust for **discrete network correspondence** and weak for **continuous spatial maps** — a distinction established with spatial-autocorrelation-preserving nulls (audit 2026-06-11, F-027). Coletta 2020 resting-state networks (6/10 diagonal-argmax) and the Pagani network bridge (4/8) **survive a mouse-rotation spin null** (p=0.002 and 0.026): the network-to-network correspondence is specific. By contrast the Margulies/Huntenburg principal gradient (|r|=0.402 parcel / 0.433 region) and the Pagani subtype Δ-matrix (r=+0.55) do **not** survive a spin test (p≈0.16–0.22) — smooth maps are matchable by spatial smoothness alone, so these are modest/spatially-unexceptional, not "strong." Re-test any spatial-map correlation with `homer.eval.nulls.spin_null` / `translation_spin_null` before claiming significance. Gene-spatial patterns translate at moderate strength. Allen ISH cell-type markers (BICCN) translate for 13 of 23 markers (mean r = +0.089), strongest for interneuron and glutamatergic classes (Drd1 +0.227, Slc17a7 +0.221, Vip +0.201, Calb2 +0.199, Pvalb +0.198). Cortical layer markers (Hodge 2019) translate for 6 of 7 (mean r = +0.119). Disorder-specific signal does not survive: predicted human maps for autism, schizophrenia, bipolar and ADHD correlate at r > 0.97 (ENIGMA), so the model carries a shared psychiatric geometry rather than disorder-specific biology.
