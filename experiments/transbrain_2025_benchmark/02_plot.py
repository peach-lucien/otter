"""Visualise the OTTER × TransBrain 2025 methods-comparison."""
...
def main():
    ...
    ax.set_xticks([0, 1])
    ax.set_xticklabels(["OTTER π", "permuted-π\nnull"])
    ax.set_ylabel("Distance: OTTER's predicted human centroid\n"
    ...
    ax.set_title(f"1 · Homology benchmark ({a['n_regions']} mouse regions)\n"
                 f"OTTER lands near the literature homolog (p < 0.001)\n"
    ...
    ax = axes[1]
    labels = ["OTTER\nvs human", "TransBrain\nvs human", "OTTER vs\nTransBrain"]
    grad = [g["otter_vs_reference"], g["transbrain_vs_reference"],
            g["otter_vs_transbrain"]]
    x = np.arange(3)
    ...
    ax.set_ylabel("OTTER ASD risk score")
    ...
    plt.suptitle("OTTER × TransBrain 2025, methods-landscape comparison "
