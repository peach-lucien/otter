"""OTTER × TransBrain 2025, advanced comparison (four follow-up analyses).

Building on `01_transbrain_benchmark.py`, this digs past the head-line
"moderate agreement" number into *where* and *why* OTTER and TransBrain agree.

  1. Trust-stratified agreement, does OTTER↔TransBrain agreement on *where a
     mouse region maps* track OTTER's own per-parcel trust score?
...
distance between OTTER's and TransBrain's top human Brainnetome region for a
given mouse region (a correlation of the full maps is dominated by OTTER's
...
def main():
    print("=" * 80)
    print("OTTER × TransBrain 2025, advanced comparison")
    print("=" * 80)
    pi = np.load(PI_FILE)
    ...
    # OTTER's region→region operator
    otter_map, region_trust, region_tier = {}, {}, {}
    for X in MOUSE_REGIONS:
        idx = np.where(parcel_acr == X)[0]
        if len(idx) == 0:
            continue
        otter_map[X] = aggregate_bn(pi[idx].sum(axis=0), bn_id, id2name, "sum")
    ...
    print("\n[1+4] Per-region OTTER↔TransBrain agreement (top-region distance)")
    dist = {}
    for X in otter_map:
        tb_col = {r: tb_mat.loc[r, X] for r in tb_mat.index if r in centroid}
        hm = {r: v for r, v in otter_map[X].items() if r in centroid}
    ...
    print(f"  top-region distance vs OTTER trust: r = {r_trust:+.3f}  ρ = {rho_trust:+.3f}"
          f"  ({len(dist)} mouse regions), negative = methods agree more where OTTER is confident")
    tier_means = {}
    ...
    for name, ph in phenos.items():
        ...
        otter_cyc = corr(np.array([ph[a] for a in common]),
        ...
        cycle[name] = {"otter": otter_cyc, "transbrain": tb_cyc}
        print(f"  {name:10s} OTTER {otter_cyc:+.3f}   TransBrain {tb_cyc:+.3f}")
    ...
    if NEUROSYNTH_DIR.exists():
        ...
        otter_bn = aggregate_bn(route_fwd(mvec, pi, np.isfinite(mvec)),
                                bn_id, id2name, "mean")
        ...
        for tp in terms:
            ...
            kh = [r for r in tv if r in otter_bn]
            kt = [r for r in tv if r in tb_bn]
            ch = corr(np.array([otter_bn[r] for r in kh]),
