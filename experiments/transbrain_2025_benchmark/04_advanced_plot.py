...
def main():
    ...
    # --- Panel 1: bidirectional cycle-consistency (the HOMER win) ----------
    ax = axes[0]
    cyc = j["cycle_consistency"]
    names = list(cyc)
    x = np.arange(len(names))
    homer = [cyc[n]["homer"] for n in names]
    tb = [cyc[n]["transbrain"] for n in names]
    ax.bar(x - 0.2, homer, 0.4, color="#2a9d8f", label="HOMER")
    ax.bar(x + 0.2, tb, 0.4, color="#e76f51", label="TransBrain")
    for i, v in enumerate(homer):
    ...
    if opto:
        top_h = opto["homer_top10"]
        tb_set = set(opto["transbrain_top10"])
        sc = opto["homer_scores"]
        ...
        ax.set_title(f"2 · Mouse AI optogenetic circuit → human cognition\n"
                     f"HOMER's top-10 terms (green = shared with TransBrain,\n"
