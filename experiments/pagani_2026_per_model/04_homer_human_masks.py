...
Here we:
  ...
  2. Route each prominent region through π (transport-weighted) → which HUMAN
     Yeo-7/subcortical networks does HOMER actually send it to? (the region-by-
...
This uses HOMER's *validated* strength (discrete region/network correspondence,
which survives spin nulls), not the continuous-map correlation that does not.

Usage:
    PYTHONPATH=src python experiments/pagani_2026_per_model/04_homer_human_masks.py
"""
...
def main():
    ...
    print("(does HOMER agree with Pagani's name-matched human homologue?)")
    print("=" * 78)
    agree = 0
    region_rows = []
    ...
    out_path = ROOT / "outputs" / "logs" / "pagani_homer_human_masks.json"
    out_path.write_text(json.dumps(out, indent=2))
    print(f"\nWrote {out_path}  (feeds 05_abide_homer_subtyping.py)")
