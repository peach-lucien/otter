"""STRIATUM-1: Add Voorn 2004 striatum subdivision anchors.

The default Caudoputamen → caudate Beauchamp anchor uses ALL 149 mouse
CP parcels mapped to a single human caudate ball. Voorn 2004 (TINS,
1198 cit) reframes the striatum as a mediolateral functional gradient.
This pack implements that:

  pid 47: Mouse CP dorsolateral subset (~18 parcels) ↔ Human putamen (sensorimotor)
  pid 48: Mouse CP ventromedial subset (~46 parcels) ↔ Human caudate ant (limbic)

Outputs:
  - outputs/coupling/pi_fc_plus_SC_with_striatum.npy
  - outputs/logs/beauchamp_validation_striatum.json

Empirically: Beauchamp Caudoputamen rises 13 → 33 % top-1 (+19 pp);
NAc rises +4 pp; ACG rises +4 pp. No off-target costs.

Usage:
    PYTHONPATH=src python experiments/anchor_packs/striatum.py
"""
from __future__ import annotations

import json
import os
import subprocess
import sys
from pathlib import Path

import numpy as np

ROOT = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(ROOT / "src"))

from homer.data import load_cached                                    # noqa: E402
from homer.data.anchor_packs import build_striatum_region_anchors    # noqa: E402
from homer.models import MultimodalFGW                                # noqa: E402

ANN = ROOT / "outputs" / "anndata"
COUP = ROOT / "outputs" / "coupling"
LOG = ROOT / "outputs" / "logs"


def main():
    M, _ = load_cached("mouse", cache_dir=ANN)
    H, _ = load_cached("human", cache_dir=ANN)
    costs = np.load(ANN / "full_costs.npz")
    entries = build_striatum_region_anchors(M.var, H.var, atlas_root=ROOT)
    print("Striatum anchors:")
    for e in entries:
        print(f"  pid={e.pair_id:>3d}  {e.label!r}  |m|={len(e.mouse_indices)}  "
              f"|h|={len(e.human_indices)}")

    model = MultimodalFGW(use_sc=True, sc_weight=0.3, fc_weight=0.7,
                          epsilon=5e-3, xyz_weight=0.5, lam_anchor=1.0)
    model.fit(M, H, Cm_SC=costs["Cm_SC"], Ch_SC=costs["Ch_SC"],
              region_anchors=entries)
    out = COUP / "pi_fc_plus_SC_with_striatum.npy"
    np.save(out, model.pi)

    env = {**os.environ, "PYTHONPATH": str(ROOT / "src")}
    subprocess.run(
        ["python", str(ROOT / "pipeline" / "05f_beauchamp_validation.py"),
         "--pi-file", out.name],
        env=env, check=True, capture_output=True,
    )
    src = LOG / "beauchamp_validation.json"
    dst = LOG / "beauchamp_validation_striatum.json"
    if src != dst:
        src.rename(dst)
    d_str = json.loads(dst.read_text())

    # Re-run production
    subprocess.run(["python", str(ROOT / "pipeline" / "05f_beauchamp_validation.py"),
                    "--pi-file", "pi_fc_plus_SC.npy"], env=env, check=True, capture_output=True)
    d_prod = json.loads((LOG / "beauchamp_validation.json").read_text())

    print(f"\n{'region':6s} {'prod':>7s} {'+striat':>10s} {'Δ':>7s}")
    for short, name in [
        ('Cau', 'Caudoputamen -> caudate nucleus'),
        ('NAc', 'Striatum ventral region -> nucleus accumbens'),
        ('ACG', 'Anterior cingulate area -> cingulate gyrus'),
        ('Mot', 'Primary motor area -> precentral gyrus'),
        ('Thal','Thalamus -> thalamus'),
        ('S1',  'Primary somatosensory area -> postcentral gyrus'),
    ]:
        p = d_prod.get(name, {}).get('top1')
        s = d_str.get(name, {}).get('top1')
        if p is None or s is None: continue
        print(f"  {short:4s} {p*100:>6.0f}%  {s*100:>8.0f}%  {(s-p)*100:>+5.0f}pp")


if __name__ == "__main__":
    main()
