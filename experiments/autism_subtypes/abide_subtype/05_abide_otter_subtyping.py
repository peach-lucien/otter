"""ABIDE re-subtyping with OTTER-derived masks vs Pagani's name-matched masks.
...
def aal_regions_for_otter_mask(mask_parcel_idx, H_xyz, aal_centroids):
    """An AAL region is 'in' the OTTER mask if its nearest OTTER human parcel is."""
    ...
    nearest = d2.argmin(axis=1)                      # AAL region -> OTTER parcel
...
def main():
    ...
    results = {}
    for scheme in ("otter", "name"):
        if scheme == "otter":
            hypo_aal = aal_regions_for_otter_mask(masks["masks"]["hypo"]["parcel_indices"], H_xyz, aal_centroids)
            hyper_aal = aal_regions_for_otter_mask(masks["masks"]["hyper"]["parcel_indices"], H_xyz, aal_centroids)
        else:
    ...
    lh = np.array(results["otter"]["labels"]); ln = np.array(results["name"]["labels"])
    agree = float((lh == ln).mean())
    print(f"\nPagani reports ~25% subtyped (7.9% hypo / 17.2% hyper). "
          f"OTTER-mask subtyped {results['otter']['pct_subtyped']}%, name-mask {results['name']['pct_subtyped']}%.")
    print(f"Label agreement OTTER vs name-matched: {agree:.2f}")
    out = {"results": {k: {kk: vv for kk, vv in v.items() if kk != 'labels'}
                       for k, v in results.items()},
           "label_agreement": agree, "pagani_reference_pct": 25.1}
    (LOG / "abide_otter_subtyping.json").write_text(json.dumps(out, indent=2))
    print(f"Wrote {LOG/'abide_otter_subtyping.json'}")
