...
**OTTER's per-disorder predictions are essentially identical at parcel resolution.** This matches the cross-disease specificity finding at network level and is stronger at parcel level (mean off-diagonal +0.993). OTTER captures a generic brain-disorder spatial geometry rather than disorder-specific signals.

Psoriasis (skin disease, non-brain) was excluded because only 2 of its 18 genes overlap with OTTER's panel, so the non-brain control cannot be tested directly. The consistency across the 4 brain disorders that can be tested is evidence that OTTER does not discriminate at this level.

## Phase 2. ENIGMA comparison (needs external data)

Testing OTTER's generic prediction against ENIGMA's observed cortical-thickness Cohen's d maps per disorder requires the ENIGMA Toolbox summary statistics:
...
Given Phase 1's result, OTTER-predicted patterns are expected to correlate similarly with every ENIGMA disease map (diagonal ≈ off-diagonal), because the predictions do not differ between disorders. Phase 2 answers a separate question, how far OTTER's generic brain-disorder geometry matches what is observed across psychiatric ENIGMA disorders. A high overall correlation would mean OTTER captures the shared "psychiatric perturbation territory" in human cortex.
...
## Disorder-unique + transdiagnostic, `04_disorder_unique.py`, `05_transdiagnostic.py`

The r=0.988 "no specificity" result is not an effect of gene-set overlap.
`04_disorder_unique.py` strips each disorder to the genes unique to it and re-routes.
The non-autism sets are essentially nested in the 1,713-gene autism set, so the test is
pairwise and relative-unique (genes in A-not-B vs B-not-A). Fully disjoint sets
(bipolar-only 26 genes vs SCZ-only 447 genes) still route to near-identical human maps
(r=+0.98), so the shared psychiatric geometry is not an overlap artefact.

`05_transdiagnostic.py` compares OTTER's generic predicted map to ENIGMA observed
cortical-thickness Cohen's d (DK-68), including a transdiagnostic average across
ASD/SCZ/BD/ADHD and a spin null over DK centroids. The correlation is r=−0.27 against
the transdiagnostic average, spin p=0.24 (n.s.), and n.s. for every individual disorder
and the held-out MDD/OCD. OTTER's shared geometry is a property of the gene→π routing
and does not align with the observed ENIGMA thickness signature beyond spatial
autocorrelation, a negative result against external maps. (ENIGMA CSVs: ENIGMA Toolbox `summary_statistics`, staged in
