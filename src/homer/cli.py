...
After ``pip install -e .``:
    ...
    homer-solve [--config fc_plus_SC] [--multistart]
    homer-evaluate [--recompute] [--skip 05c_null_distributions.py]
    homer-artefacts
...
def _pipeline_dir() -> Path:
    """Locate homer/pipeline/. Works whether installed editable or sourced."""
    ...
    for parent in (here.parents[2], here.parents[3], here.parents[4]):
        ...
        if cand.exists():
            return cand
    raise RuntimeError("Could not locate homer/pipeline/ relative to homer package")
...
def solve_production() -> None:
    """Entry point for ``homer-solve``."""
    sys.exit(_run("04_solve_production.py"))


def evaluate() -> None:
    """Entry point for ``homer-evaluate``."""
    sys.exit(_run("05_evaluate.py"))


def build_artefacts() -> None:
    """Entry point for ``homer-artefacts``."""
