"""The production recipe for fitting a HOMER coupling.
...
Typical use::
    ...
    from homer.repro import load_inputs, anchor_warped_xyz, fit_coupling, CANONICAL
...
def repo_root() -> Path:
    """The `homer/` directory, however the caller happens to be invoked."""
...
def beauchamp_scorer(root: Path | None = None):
    ...
    Loaded by path because it still lives under ``experiments/``. It belongs in ``homer.eval``;
    ...
    spec = importlib.util.spec_from_file_location("homer_beauchamp_battery", path)
...
def provenance(name: str = "pi_canonical.npy") -> dict:
    ...
    """
    from homer.data import pi_provenance
