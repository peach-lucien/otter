"""Fetch OTTER's data + artifacts from the versioned Zenodo archive.
...
def find_root(start: Path | None = None) -> Path:
    ...
    working directory. Falls back to the package's parents[3] (src/otter/data ->
...
def ensure_data(required, *, tier: str = "reproduce", what: str = "this step",
    ...
    * with ``OTTER_AUTO_FETCH=1`` set, download the tier automatically;
    ...
    * else (or if they decline), raise :class:`DataNotFound` with the exact
    ...
    auto = os.environ.get("OTTER_AUTO_FETCH", "").strip().lower() in ("1", "true", "yes")

    if not auto and sys.stdin.isatty() and sys.stdout.isatty():
        print(f"\nOTTER needs data for {what} that isn't present yet:\n{listing}")
        resp = input(
    ...
    if not auto:
        raise DataNotFound(
            ...
            f"Or set OTTER_AUTO_FETCH=1 to download automatically when needed. "
            f"See DATA.md."
        )
