...
def _import_plotly():
    """Lazy-import plotly so the rest of otter doesn't require it for non-viz use."""
    try:
        import plotly.graph_objects as go
    except ImportError as e:                                                # pragma: no cover
        raise ImportError(
            "otter.viz.notebook requires plotly; install with `pip install plotly`."
        ) from e
    return go
...
def plot_per_network_heatmap(long_df, *,
    ...
    (the format produced by ``otter.viz.reports.build_per_network_long``).
    """
    go = _import_plotly()
...
def plot_comparison_bars(table_df, *,
    ...
    the wide-form table produced by ``otter.viz.reports.build_comparison_table``.
    """
    go = _import_plotly()
    from plotly.subplots import make_subplots
