...
from otter.data.networks import assign_networks, network_mismatch_mask
from otter.models._solver import entropic_semirelaxed_fgw_multistart
from otter.models.base import FitInfo
from otter.data.region_anchors import apply_region_supervision
from otter.models.supervised import SupervisedFGW, _apply_anchor_supervision, _build_xyz_M
...
class MultimodalFGW(SupervisedFGW):
    ...
    use_sc : bool, default True
        ...
        :func:`otter.costs.sc_correlation_distance`).
    ...
    M_anchor_weight : float, default 0.0
        Anchor-relationship cross-species cost in M. Computed per-fold from
        the visible anchors only.
    ...
    def _solve(self, *, mouse_ad, human_ad,
               ...
               # Region-anchor supervision: list of RegionAnchorEntry
               ...
               # Outside-region penalty for region anchor constraints. The
               # default 0.15 gives better-calibrated probability
               # distributions at no cost to argmax / top-K predictions. Pass
               # 1.0 (or None) for hard 0/1 wall behaviour.
               ...
               # in M. Per-row weighting does not reproduce the global xyz
               # effect, because the FGW equilibrium is non-local. Pass None
        ...
        # Region-anchor supervision, applied after point anchors. The
        ...
        return pi, FitInfo(
            loss=loss, n_iter=n_iter, converged=converged,
            extra={"weights": weights, "n_visible_anchors": len(visible),
                   "final_err": float(log["err"][-1]) if log.get("err") else None},
        )
