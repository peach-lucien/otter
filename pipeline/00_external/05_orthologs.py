...
def main():
    ...
    if len(mouse_meta) == 0:
        ...
        print("If the Allen ISH download keeps returning 'data_set.xml' only, try the")
        print("fallback: PYTHONPATH=src python scripts/external/02b_mouse_genes_direct.py")
        sys.exit(1)
    if len(human_meta) == 0:
