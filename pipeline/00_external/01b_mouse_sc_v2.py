"""Build mouse_sc.npy using v2 NS voxel centres directly.

Supersedes the legacy v1 heuristic-transform path (removed; see the
``archive/v1-pipeline`` branch). Reads
``ns_center_ix`` from corrs_mouse_v2.mat via the new loader and looks up
the CCFv3 25 µm annotation at that voxel directly.

For the summary-structure SC matrix itself we still use the Allen Mouse
Connectivity Atlas (Oh et al. 2014) summary-level unionised projection
volumes via AllenSDK — that piece doesn't change. What changes is the
parcel→summary-structure assignment: under v1 it went through the
brute-force 48-permutation transform; under v2 it goes through Paul's
nonlinear DSURQE→CCFv3 warp via ``ns_center_ix``.

Outputs (overwrites the legacy artefact):
...
def main(resolution_um: int = 100):
    ...
    # Note: under v2 we ALSO have a shipped 25 µm CCFv3 annotation at
    # `data_crossspecies/updated_connectom_0906_26/ANO_ABA_NS.nii.gz` — but
    # we still use the AllenSDK 100 µm version for the structure-tree
    # mapping (descendant_ids), which is independent of voxel-level lookups.
    ...
    # (Unchanged from v1 — this is independent of parcel placement.)
    ...
    # -- 3. Project each of 1864 nodes onto a CCFv3 region via v2 ----------
    print("projecting 1864 mouse nodes onto CCFv3 regions via v2 ns_center_ix...")
    meta = IO.load_metadata("mouse")
    if meta["_schema"] != "v2":
        print(f"  WARNING: schema is {meta['_schema']!r}; this script expects v2. "
    ...
    meta_out = {
        ...
        "symmetrised":       True,
        "ns_source": "v2 ns_center_ix (Paul's nonlinear DSURQE→CCFv3 warp)",
    }
