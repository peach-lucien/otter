"""Build mouse_genes.npy using v2 NS voxel sets directly.

Supersedes the legacy v1 heuristic-transform gene-sampling path (removed;
see the ``archive/v1-pipeline`` branch). Reads ``AS_ix`` (Allen CCFv3 25 µm voxel
indices) from ``corrs_mouse_v2.mat`` via the new loader and samples Allen
ISH energy volumes at 200 µm at the corresponding downsampled voxels.
...
For each Allen ISH gene:
  ...
  3. For each of the 1864 parcels, take the parcel's NS voxel set
     (from the v2 loader, 25 µm grid), downsample each voxel to the
     200 µm grid (integer divide each axis index by 8), deduplicate,
     and average the energy values.
  4. Stack per-parcel means into a (1864, n_genes) matrix.

Outputs (overwrites the legacy artefacts; the previous warp-rebuild
outputs at `data_external/mouse_genes_warped.npy` can now be retired):
...
Backwards compatibility:
    ...
    ``mouse_genes.npy`` and ``mouse_gene_list.csv`` only; they do not care
    that the sampling now goes through Paul's nonlinear warp via v2 AS_ix
    rather than the old 48-permutation heuristic.
"""
...
def ns_voxels_to_200um(ns_indices_25um: np.ndarray,
    ...
    The v2 loader produces 0-based linear indices in Fortran order into the
...
def main():
    IO = _load_io()
    print(f"Loading v2 metadata via {IO.__name__}...")
    meta = IO.load_metadata("mouse")
    schema = meta["_schema"]
    if schema != "v2":
        print(f"WARNING: schema is {schema!r}; this script expects v2. "
              f"Point DATA_DIR at the updated_connectom_0906_26/ package.")
    ...
    work_dir = DATA_EXT / "_ish_cache_v2"
    work_dir.mkdir(parents=True, exist_ok=True)
    ...
    meta_out = {
        "source": "Allen ISH energy at 200 µm sampled via v2 AS_ix (Paul's nonlinear DSURQE→CCFv3 warp).",
        ...
        "ns_voxel_source": "v2 corrs_mouse_v2.mat AS_ix (25 µm) → integer-divide by 8 → 200 µm",
    }
